...
> [!WARNING]
> **Not Official USMC Software.** Semper Scribe is a non-official Proof of Concept maintained on a personal basis. It is not USMC, DON, or DoD software and carries no Authority to Operate. Use is at the user's discretion and risk. All processing happens in the browser. **Never enter classified information. Whether CUI is permitted depends on the system you run it on and on your authorizing official's approval of this application for that use (DoDI 5200.48 paragraphs 3.3.c and 3.10.b). Until your command has granted that approval, do not enter CUI.** PII such as names and EDIPI is accepted at the user's discretion and risk: the formatter never transmits it, and every export is scanned for SSN and EDIPI patterns before the file is written. Personal information about a real service member, once part of a record, is CUI (Privacy, marking PRVCY) under the DoD CUI Registry, so a form produced for a real Marine falls under the CUI condition above. See the [Privacy and Security Notice](https://semperadmin.github.io/semperscribe/privacy) and [SECURITY.md](SECURITY.md) for full details.
