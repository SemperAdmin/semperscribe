...
/**
 * S2f (Stephen's walk, 2026-06-10): one action here — configure the
 * fields. Download and request-link moved to where they are used:
 * the PDF Export carries configured fields automatically, and the
 * placement modal owns Save & Copy Request Link. CAC PKI only
 * (ruling): the signer's Acrobat step is the floor (plan K1).
 */
    ...
    <Card className="shadow-sm border-border mb-6 border-l-4 border-l-primary">
      ...
      <CardContent className="space-y-4 pt-4">
        <p className="text-sm text-muted-foreground">
          Place CAC/PKI signature fields on the PDF. Configured fields ride the exported PDF and the signature request link automatically.
        </p>
        ...
        {signatureFields.length > 0 ? (
          ...
          <p className="text-xs text-muted-foreground italic">
            No fields configured yet — exported PDFs are unchanged until you place one.
          </p>
        )}
        <p className="text-xs text-muted-foreground italic">
          Copy the request link from the placement screen and paste it into your e-mail. The signer signs in Adobe Acrobat with their CAC and returns the signed file — the link carries the request, never the signature.
        </p>
      </CardContent>
    </Card>
  );
