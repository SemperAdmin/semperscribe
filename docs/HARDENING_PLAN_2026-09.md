...
## Phase B.5 and B.6 status, 2026-09-05: landed (v0.4.5)
...
## Phase C.2 status, 2026-09-05: landed (v0.5.0)

The companion process. `companion/` at the repository root, outside `src/` so `next build` never sees it, holds four pure operations in `handler.ts` and two front doors over them: `server.ts` on `127.0.0.1:7719` with `GET /health`, `GET /document-types`, `POST /validate`, and `POST /render`, and `mcp.ts` on stdio with `list_document_types`, `get_document_schema`, `validate_document`, and `render_document`. Both run with `tsx`. `getDocumentSchema` generates the form-data half from the type's own zod schema through `z.toJSONSchema`, so an agent reads what the application validates rather than a hand-kept copy.

`renderDocument` reproduces the editor's export sequence rather than reimplementing it: NLDP structure and integrity, the letter validators, the export gate's sensitive-data scan, the SECNAV five page cap, and the same pipeline selection, including the branch which fills the official NAVMC blanks and the I-Type route through `exportDocument`. Findings from the scan refuse the render with a 422 naming them, and only an explicit `acknowledgeSensitive` proceeds. Refusing rather than prompting is the whole difference between the two surfaces, since a headless caller has no dialog to answer.

Three limits, each with its own test. Two megabyte body cap, checked against the declared Content-Length first and then counted as chunks arrive. Forty five second timer race per operation, real timer, cleared on both outcomes. Output writes confined to `COMPANION_OUT_DIR` through realpath, with traversal, absolute paths, planted symlinks, and symlinked subdirectories each refused and each pinned by a test.

Sixty tests across six files under `tests/companion/`, all in the plain Node environment. The one which matters most is `pdf-parity.test.ts`: it packs the frozen golden fixture into an NLDP package, renders it through the companion, and compares the positioned text layout against the same committed snapshot `tests/golden/pdf-golden.test.ts` measures the browser pipeline on. It matches exactly. A future divergence there means the companion has drifted off the application's path through the pipeline, which is the failure mode a contract test alone would miss.

Suite 2,560 passing (the LibreOffice half of the pagination parity test still needs the CI runner). Initial-load JS 2,554,445 B to 2,554,573 B, plus 128 bytes. None of it is companion code: `src/lib/nldp-utils.ts` imports the app version from `package.json` and the bundler inlines the whole manifest, so the two new dependency lines and the two new script lines ride along. Total 6,891,678 B. Both budgets hold with wide margin.
