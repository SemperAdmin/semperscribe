...
| D-45 | The app locks what the form forgot to lock | DECIDED 2026-08-25, UNBUILT, mitigation for defect 3.9. THE DECISION STANDS: once `9 NJP AUTHORITY SIGNATURE` carries a `/V`, the app should treat items 6, 6 date, 8, 8A, 8B and 10 as APP-LOCKED even though the form leaves them writable, and should record their values so a later pass detects an outside change. WHAT THE CODE DOES TODAY: nothing of this, and the second half was never possible. It depends on a "carried state" that DOES NOT EXIST: the export writes AcroForm field values and nothing else, no embedded file, no `/Names`, no metadata, no XMP. V-27 does not exist. Defect 3.9 is unmitigated |
